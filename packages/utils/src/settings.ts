...
const multipleRegex = makeValidator((x) => {
  ...
  patterns.forEach((p) => {
    try {
      ...
      if (delimiterIndex !== -1) {
        p = p.slice(delimiterIndex + 4); // Corrected slicing to include the delimiter
      }
      new RegExp(p);
    } catch (e) {
...
export const Settings = cleanEnv(process.env, {
  ...
  DEFAULT_EASYNEWS_PLUS_PLUS_TIMEOUT: num({ // Added missing default
    default: undefined,
    desc: 'Default Easynews++ timeout',
  }),
